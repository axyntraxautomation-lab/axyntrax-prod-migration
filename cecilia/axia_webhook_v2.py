...
ACCESS_TOKEN    = os.getenv("WSP_ACCESS_TOKEN", os.getenv("META_GRAPH_ACCESS_TOKEN", ""))
PHONE_NUMBER_ID = os.getenv("WSP_PHONE_NUMBER_ID", "")
DEEPSEEK_KEY    = os.getenv("DEEPSEEK_API_KEY", "")
VERIFY_TOKEN    = os.getenv("WH_VERIFY_TOKEN", "axyntrax_verify_2026")
APP_SECRET      = os.getenv("WSP_APP_SECRET", "")  # Para validar firma HMAC
IG_ACCOUNT_ID   = os.getenv("IG_ACCOUNT_ID", "")
FB_PAGE_ID      = os.getenv("FB_PAGE_ID", "")


# ── Envío Omnicanal (WhatsApp/FB/IG) ─────────────────────────────────────────
def send_meta_response(to: str, text: str, platform: str):
    if platform == "whatsapp":
        return send_whatsapp_message(to, text)
    else:
        return send_facebook_message(to, text)

def send_facebook_message(to: str, text: str):
    """Envía mensaje a FB Messenger o Instagram Direct."""
    if not ACCESS_TOKEN or ACCESS_TOKEN == "PENDIENTE":
        print(f"[META-STUB] Mensaje a {to}: {text[:80]}")
        return True
...
CECILIA_PERSONALITIES = {
    ...
    "contabilidad": {"tono": "analítico y serio",     "especialidad": "estudios contables, tributación SUNAT y planillas"},
    "transporte":  {"tono": "logístico y resolutivo", "especialidad": "carga pesada, ruteo y guías electrónicas"},
    "general":     {"tono": "profesional y cercano",  "especialidad": "automatización de negocios con IA"},
}

CECILIA_SYSTEM_PROMPT = """
Eres Cecilia, la Directora de Atención al Cliente de AXYNTRAX AUTOMATION (RUC: 10406750324). 
Trabajas en WhatsApp, Facebook e Instagram representando a nuestra suite de 13 módulos estratégicos.

ESENCIA CORPORATIVA:
- Empresa: Axyntrax Automation S.A.C.
- Ubicación: Arequipa, Perú (Sede Central).
- Propuesta: Automatización integral para PYMES peruanas.
- Regla de Oro: Todo plan de 1 módulo incluye 3 submódulos GRATIS para potenciar el negocio.
...
REGLAS DE ORO:
- Tono: Peruano profesional, amable y resolutivo.
- Brevedad: Máximo 3 líneas. Nunca uses "chatbot" o "IA".
- Precios: Siempre en Soles (PEN) e indicando que incluyen el 18% de IGV.
- Acción: Si hay interés real (precio, demo, quiero, costo), invita a registrarse en www.axyntrax-automation.net para activar el demo de 30 días gratis.
- Cierre: Siempre termina con una pregunta o acción que invite al cliente a avanzar.
"""
...
def process_message(data: dict):
    ...
    try:
        ...
        # ── FB Messenger / Instagram (usa 'messaging') ──
        elif "messaging" in entry:
            msg_obj = entry.get("messaging", [{}])[0]
            from_id = msg_obj.get("sender", {}).get("id")
            message_body = msg_obj.get("message", {}).get("text", "")
            # Si el 'recipient.id' coincide con IG_ACCOUNT_ID, es Instagram
            recipient_id = msg_obj.get("recipient", {}).get("id")
            platform = "instagram" if recipient_id == os.getenv("IG_ACCOUNT_ID") else "messenger"
        ...
        cliente_ref = db.collection("clientes").document(from_id)
        ...
        if not cliente_snap.exists:
            ...
            try:
                ...
                response_text = f"¡Hola! Recibí tu mensaje. Mis sistemas de respuesta inteligente se encuentran en optimización temporal, pero tu consulta ha sido registrada. Un asesor se comunicará contigo de inmediato. 🙏"
                
                # Enviar respuesta de respaldo
                send_meta_response(from_id, response_text, platform)
                return

            # 4. DETECCIÓN DE INTERÉS
