...
def health():
    ...
    deepseek_present = bool(os.getenv("DEEPSEEK_API_KEY") and os.getenv("DEEPSEEK_API_KEY") != "PENDIENTE_POR_ADMIN")
    use_ai = os.getenv("USE_AI", "true").lower() == "true"
    return jsonify({
        "status": "operational",
        "whatsapp": wsp_ok,
        "gemini_present": gemini_present,
        "deepseek_present": deepseek_present,
        "use_ai": use_ai,
        "ai_active": (gemini_present or deepseek_present) and use_ai,
        "version": "3.2.0",
...
def jarvis_notificar():
    ...
    # INTEGRACIÓN CON HUNTER (Growth Bot)
    if tipo in ("LEAD", "MENSAJE"):
        try:
            hunter = get_hunter()
            metadata = body.get("metadata", {})
            sender_id = metadata.get("senderId", "REMOTE_USER")
            hunter.process_new_interaction("REMOTE_SESSION", {
                "sender": sender_id,
                "text": mensaje
            })
        except Exception as hunter_err:
            print(f"[HUNTER INTEGRATION ERR] {hunter_err}")
...
# GROWTH (HUNTER)
# ══════════════════════════════════════════════════════════════════════════════
@app.route("/api/growth/leads", methods=["GET"])
def list_leads():
    try:
        conn = get_db()
        c    = conn.cursor()
        c.execute("SELECT id, source, score, rubro, estado, creado_en FROM leads_v6 ORDER BY score DESC, creado_en DESC LIMIT 50")
        rows = [dict(zip(["id","source","score","rubro","estado","creado_en"], r))
                for r in c.fetchall()]
        conn.close()
        return jsonify({"ok": True, "leads": rows, "count": len(rows)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    deepseek_present = bool(os.getenv("DEEPSEEK_API_KEY") and os.getenv("DEEPSEEK_API_KEY") != "PENDIENTE_POR_ADMIN")
    ai_active = (gemini_present or deepseek_present) and use_ai
    print(f"[AXYNTRAX UNIFIED API v3.2.0] Puerto {port} | Auth: ON | DeepSeek: {deepseek_present} | Gemini: {gemini_present} | Active: {ai_active}")
