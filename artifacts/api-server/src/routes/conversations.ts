...
router.get("/conversations", requireAuth, async (req, res) => {
  ...
  if (typeof req.query.assignedAgentId === "string") {
...
router.post("/conversations/:id/messages", requireAuth, async (req, res) => {
  ...
  if (conv.channel === "gmail" && conv.contactHandle) {
    // Gmail threads new messages by `threadId` AND requires the Subject to
    // match (case-insensitive, leading "Re: "/"Fwd: " optional). We stored
    // the gmail thread id as `gmail:<threadId>` in `externalId` at ingest
    // time, and the original Subject in `conv.subject` — preserve both as-is.
    const threadId =
...
router.post("/conversations/:id/link-client", requireAuth, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Invalid id" });
    return;
