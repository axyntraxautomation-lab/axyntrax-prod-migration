...
export default function AxiaCentralPage() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
          ...
          try {
            ...
            if (chatRes.ok) {
              ...
              if (attempt < maxRetries) {
                console.warn(`[AXIA Chat] Intento ${attempt} fallido. Reintentando...`)
            ...
            attempt++
            if (attempt < maxRetries) {
              console.warn(`[AXIA Connection] Error de conexion en intento ${attempt}. Reintentando...`)
              await delay(attempt * 1000)
            } else {
              break
            }
      ...
      console.error('[AXIA Chat Loop Error]', e)
    }

    if (!success) {
      // ── Sin backend o fallaron reintentos: respuesta offline inteligente ────────────────────
