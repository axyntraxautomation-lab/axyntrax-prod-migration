...
- **Dominio descontinuado:** `axyntrax-automation.net
` quedó desreferenciado del código en abril de 2026 (pivote del fundador a `.net`). El `.com` sigue registrado en Namecheap pero el código nuevo ya no lo menciona; el dueño decide después si lo deja vencer o lo redirige.
...
- **Databases:**
    ...
    - Supabase PostgreSQL — DB independiente para el SaaS multi-tenant **Cecilia** (paquete `@workspace/db-supabase`, IDs `uuid` nativo). 18 tablas con prefijo `tenant_*` + `tenants` + `rubros_registry`. Conexión vía pooler con TLS verify-full obligatorio: el CA root público de Supabase (Supabase Root 2021 CA, vence 2031-04-26) está embebido en el código (`lib/db-supabase/src/certs/embedded.ts`); no hay env var para deshabilitar verificación. Nunca se mezcla con la DB Replit (paquetes y schemas separados). **Setup obligatorio antes de usar `/api/tenant/jwt`:** `pnpm --filter @workspace/db-supabase run bootstrap` provisiona idempotentemente el rol PG `tenant_owner` (NOLOGIN) y lo otorga a `authenticator` para que PostgREST acepte el JWT firmado por el bridge. Las policies RLS por tenant llegan en task #45.
